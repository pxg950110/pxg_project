<template>
  <PageContainer :title="`量表设计器 · ${meta.name || '新建量表'} · 页面原型`" :breadcrumb="[{ title: '原型' }, { title: '量表设计器' }]">
    <template #extra>
      <a-button @click="$router.push({ name: 'ScaleManagement' })">返回列表</a-button>
      <a-button @click="previewOpen = true">预览</a-button>
      <a-button type="primary" :loading="publishing" @click="handlePublish">发布新版本</a-button>
    </template>
    ...
    <div class="designer">
      ...
      <div class="canvas" @dragover.prevent @drop="onCanvasDrop">
        <div class="pane-title">
          量表画布（{{ items.length }} 题 · 计分题 {{ scoreItemCount }}{{ (meta.maxScore || autoMaxScore) ? ' · 满分 ' + (meta.maxScore || autoMaxScore) : ' · 不计分' }}）
        </div>
...
const BINDING_DOMAIN_META: Record<string, { label: string; color: string }> = {
...
/* 初始化：带 scaleCode 参数则从后端载入既有量表定义（immediate 兼容异步路由注册） */
