# Task 05: EtlPalette Component Panel

**Files:**
- Create: `src/views/data-etl/components/EtlPalette.vue`

- [ ] **Step 1: Create EtlPalette.vue**

The palette shows all available ETL components grouped by category. Users drag components from here onto the canvas.
...
.etl-palette__item:hover {
  border-color: #4096ff;
  background: #f0f5ff;
}

.etl-palette__item:active {
  cursor: grabbing;
  box-shadow: 0 2px 8px rgba(0, 0, 0, 0.12);
}

.etl-palette__item-icon {
  font-size: 14px;
  color: rgba(0, 0, 0, 0.65);
}
</style>
...
```bash
git add src/views/data-etl/components/EtlPalette.vue
git commit -m "feat(etl): add EtlPalette component panel with drag support"
```
