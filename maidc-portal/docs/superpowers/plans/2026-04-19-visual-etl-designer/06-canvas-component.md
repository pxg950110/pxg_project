# Task 06: EtlCanvas Vue Flow Canvas

**Files:**
- Create: `src/views/data-etl/components/EtlCanvas.vue`

- [ ] **Step 1: Create EtlCanvas.vue**

The canvas is the central Vue Flow component that renders the graph. It handles drop events from the palette and emits all graph interactions.
...
import type { GraphNode, GraphEdge, Connection, NodeChange, EdgeChange } from '@vue-flow/core'
...
const emit = defineEmits<{
  'node-click': [payload: { node: GraphNode<EtlNodeData> }]
  'edge-double-click': [payload: { edge: GraphEdge }]
...
function onEdgeDoubleClick(payload: any) {
  emit('edge-double-click', payload)
}
...
.etl-canvas :deep(.vue-flow) {
  background: #fafafa;
}

.etl-canvas :deep(.vue-flow__minimap) {
  border-radius: 8px;
  overflow: hidden;
}
</style>
...
```bash
git add src/views/data-etl/components/EtlCanvas.vue
git commit -m "feat(etl): add EtlCanvas with Vue Flow, Background, Controls, MiniMap"
```
