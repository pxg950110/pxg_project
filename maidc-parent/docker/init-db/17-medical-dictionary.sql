...
CREATE TABLE IF NOT EXISTS masterdata.d_drug_category (
    ...
    CONSTRAINT uk_drug_category_code UNIQUE (code)
);
...
-- 索引
CREATE INDEX idx_drug_category ON masterdata.d_drug(category_id) WHERE is_deleted = false;
CREATE INDEX idx_drug_atc ON masterdata.d_drug(atc_code) WHERE atc_code IS NOT NULL AND is_deleted = false;
CREATE INDEX idx_drug_insurance ON masterdata.d_drug(insurance_code) WHERE insurance_code IS NOT NULL;
CREATE INDEX idx_drug_status ON masterdata.d_drug(status) WHERE is_deleted = false;
CREATE INDEX idx_drug_name ON masterdata.d_drug USING gin(to_tsvector('simple', name));
...
-- 索引
CREATE INDEX idx_diagnosis_icd10 ON masterdata.d_diagnosis(icd10_code) WHERE icd10_code IS NOT NULL AND is_deleted = false;
CREATE INDEX idx_diagnosis_chapter ON masterdata.d_diagnosis(chapter_code);
CREATE INDEX idx_diagnosis_parent ON masterdata.d_diagnosis(parent_id);
CREATE INDEX idx_diagnosis_name ON masterdata.d_diagnosis USING gin(to_tsvector('simple', name));
...
CREATE TABLE IF NOT EXISTS masterdata.d_fee_item (
    ...
    is_deleted          BOOLEAN      NOT NULL DEFAULT FALSE,
    org_id              BIGINT       NOT NULL DEFAULT 0,
    CONSTRAINT uk_fee_item_code UNIQUE (fee_code)
);

COMMENT ON TABLE masterdata.d_fee_item IS '收费项目字典';

-- 索引
CREATE INDEX idx_fee_category ON masterdata.d_fee_item(fee_category) WHERE is_deleted = false;
CREATE INDEX idx_fee_insurance ON masterdata.d_fee_item(insurance_code) WHERE insurance_code IS NOT NULL;
CREATE INDEX idx_fee_name ON masterdata.d_fee_item USING gin(to_tsvector('simple', name));
...
CREATE TABLE IF NOT EXISTS masterdata.d_lab_item (
    ...
    is_deleted          BOOLEAN       NOT NULL DEFAULT FALSE,
    org_id              BIGINT        NOT NULL DEFAULT 0,
    CONSTRAINT uk_lab_item_code UNIQUE (lab_code)
);

COMMENT ON TABLE masterdata.d_lab_item IS '检验项目字典';

-- 索引
CREATE INDEX idx_lab_loinc ON masterdata.d_lab_item(loinc_code) WHERE loinc_code IS NOT NULL;
CREATE INDEX idx_lab_specimen ON masterdata.d_lab_item(specimen_type_code);
CREATE INDEX idx_lab_parent ON masterdata.d_lab_item(parent_id);
CREATE INDEX idx_lab_name ON masterdata.d_lab_item USING gin(to_tsvector('simple', name));
...
CREATE INDEX idx_lab_reference_item ON masterdata.d_lab_reference(lab_item_id);
...
CREATE TABLE IF NOT EXISTS masterdata.d_exam_item (
    ...
    is_deleted          BOOLEAN      NOT NULL DEFAULT FALSE,
    org_id              BIGINT       NOT NULL DEFAULT 0,
    CONSTRAINT uk_exam_item_code UNIQUE (exam_code)
);

COMMENT ON TABLE masterdata.d_exam_item IS '检查项目字典';

-- 索引
CREATE INDEX idx_exam_type ON masterdata.d_exam_item(exam_type) WHERE is_deleted = false;
CREATE INDEX idx_exam_body_site ON masterdata.d_exam_item(body_site_code);
CREATE INDEX idx_exam_name ON masterdata.d_exam_item USING gin(to_tsvector('simple', name));
