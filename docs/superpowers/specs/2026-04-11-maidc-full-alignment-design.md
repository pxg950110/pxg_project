...
- **无外键约束**：所有表不使用 PostgreSQL FOREIGN KEY 约束。表间关系通过业务层逻辑关联（`patient_id`、`encounter_id` 等字段保留但无 FK）。原因是医疗数据从多源系统（HIS/LIS/PACS）导入时顺序不可控，外键会导致导入失败。

## 风险

1. **设计稿变更**：.pen 文件可能还在迭代，需要锁定版本
2. **数据一致性**：无外键后需在业务层保证引用完整性（Service 层校验）
